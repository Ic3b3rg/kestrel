...
async function resolveLiveCodexExecutable(): Promise<string> {
  if (process.env.KESTREL_CODEX_EXECUTABLE !== undefined) {
    return realpath(process.env.KESTREL_CODEX_EXECUTABLE);
  }
...
describe.runIf(process.env.KESTREL_LIVE_CODEX === "1")("Codex App Server live conformance", () => {
  it("reads bounded subscription readiness through the current Codex session", async () => {
    const executable = await resolveLiveCodexExecutable();
