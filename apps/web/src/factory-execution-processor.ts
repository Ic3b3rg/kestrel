...
function promptFor(run: ClaimedFactoryExecution, workspace: FactoryFeatureWorkspace): string {
...
async function execute(
  ...
  const lifecycle = (phase: "implementation" | "verification"): CodexExecutionLifecycle => ({
  ...
  try {
    ...
    const result = await runtime.runTurn({
      ...
      onQuestion: async (question) => {
        ...
        throw new ExecutionFailure(question.code, text);
      },
    });
    if (hasPending()) throw new ExecutionFailure("stop_unconfirmed");
    signal.throwIfAborted();
    const completion = completionSchema.parse(JSON.parse(result.text));
    if (completion.status === "input_required")
      throw new ExecutionFailure(
        "input_required",
        completion.question ?? "Which requirement or authorized limit must change?",
      );
    if (completion.question !== null) throw new ExecutionFailure("invalid_response");
    ...
    for (const [index, command] of commands.entries()) {
      signal.throwIfAborted();
      const checked = await runtime.runVerification({
        ...lifecycle("verification"),
        workspaceCwd: workspace.workspacePath,
        gitDirectory: workspace.gitDirectory,
        cwd: command.cwd,
        command: [command.program, ...command.args],
        processId: `${run.id}:verification:${String(round)}:${String(index + 1)}`,
      ...
      await saveFactoryVerification(pool, run, {
        ...
        command,
        ...committed,
        outcome: checked.exitCode === 0 ? "passed" : "failed",
        exitCode: checked.exitCode,
        stdout,
        stderr,
        stdoutTruncated: checked.stdoutTruncated || Buffer.byteLength(checked.stdout) > 8192,
        stderrTruncated: checked.stderrTruncated || Buffer.byteLength(checked.stderr) > 8192,
        durationMs: checked.durationMs,
      });
      if (checked.exitCode !== 0) throw new ExecutionFailure("verification_failed");
    }
    await assertFeatureWorkspaceSnapshot(workspace, committed, { signal });
    signal.throwIfAborted();
    verified = true;
