import { CHANGE_OVERVIEW_RENDER_QUEUE, DIAGNOSTIC_QUEUE } from "./pg-boss.js";
    ...
    `
      SELECT to_regclass('pgboss.job')::text AS job_table,
             ...
             EXISTS (
               ...
               FROM pgboss.queue
               WHERE name = $2
             ) AS overview_queue_exists
    `,
    [DIAGNOSTIC_QUEUE, CHANGE_OVERVIEW_RENDER_QUEUE],
  );
  const readiness = result.rows[0];
  if (
    result.rowCount !== 1 ||
    !readiness?.job_table ||
    !readiness.diagnostic_queue_exists ||
    !readiness.overview_queue_exists
  ) {
